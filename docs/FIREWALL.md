...
How to configure your network to allow Dogecoin node traffic.

## Ports Overview

| Port | Protocol | Service | Required? |
|------|----------|---------|-----------|
| 22556 | TCP | P2P (peer network) | **Yes** - blockchain sync |
| 22555 | TCP | RPC (wallet/pool) | Depends on mode |
...
**Forward only port 22556:**

| Port | Forward to VM? | Why |
|------|----------------|-----|
| **22556** | **YES** | Blockchain sync, peer connections |
| 22555 | NO | Localhost only (pool software runs locally) |
...
|------|----------------|
| **22556** | **YES** |
| **22555** | **YES** (with authentication!) |

> **Warning:** Only expose RPC (22555) if you understand the security implications. Always use strong RPC credentials.
...
```bash
configure
set port-forward rule 1 description "Dogecoin P2P"
set port-forward rule 1 forward-to address 192.168.1.100
set port-forward rule 1 forward-to port 22556
set port-forward rule 1 original-port 22556
...
3. Configure:
   - Interface: WAN
   - Protocol: TCP
   - Destination port range: 22556-22556
   - Redirect target IP: [VM IP]
   - Redirect target port: 22556
4. Save and Apply
...
/ip firewall nat add chain=dstnat action=dst-nat \
  dst-port=22556 protocol=tcp to-addresses=192.168.1.100 to-ports=22556 \
  comment="Dogecoin P2P"
```
...
sudo ufw allow 22556/tcp comment 'Dogecoin P2P'
sudo ufw allow 22555/tcp comment 'Dogecoin RPC'  # Only if needed

# Remove a rule
sudo ufw delete allow 22555/tcp
...
```bash
sudo ufw allow 22556/tcp comment 'Dogecoin P2P'
sudo ufw allow 22555/tcp comment 'Dogecoin RPC'
sudo ufw allow ssh comment 'SSH Access'
```
...
```bash
# Check if dogecoind is listening
sudo ss -tlnp | grep dogecoind

# Expected output (mining pool mode):
# LISTEN  0  128  0.0.0.0:22556  *  users:(("dogecoind",pid=1234,fd=10))
# LISTEN  0  128  127.0.0.1:22555  *  users:(("dogecoind",pid=1234,fd=11))
```
...
**Using netcat:**
```bash
nc -zv YOUR_PUBLIC_IP 22556
# Success: "Connection to YOUR_PUBLIC_IP 22556 port [tcp/*] succeeded!"
```
...
```bash
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo | grep -E "connections|connections_in|connections_out"
```
...
4. **Check dogecoind is binding correctly:**
   ```bash
   sudo ss -tlnp | grep 22556
   # Should show dogecoind listening on 0.0.0.0:22556
   ```
...
nc -zv seed.dogecoin.com 22556
```

## Security Notes

- **Never expose RPC (22555) without authentication** on a public IP
- Mining pool mode keeps RPC on localhost (127.0.0.1) for security
- P2P port (22556) is safe to expose - it's designed for public access
...
```bash
#!/bin/bash
echo "=== Dogecoin Node Connectivity Test ==="

echo -e "\n1. Service Status:"
systemctl is-active dogecoind

echo -e "\n2. Listening Ports:"
ss -tlnp | grep dogecoind

echo -e "\n3. Peer Connections:"
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo 2>/dev/null | grep -E '"connections"|connections_in|connections_out' || echo "RPC not responding"
...
echo "Test port 22556 at: https://canyouseeme.org/"
```
