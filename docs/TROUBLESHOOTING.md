...
Common issues and solutions for the Dogecoin node setup.
...
# Check recent logs
sudo journalctl -u dogecoind -n 50

# Check sync progress
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getblockchaininfo 2>/dev/null | jq '{blocks, headers, verificationprogress, initialblockdownload}'

# Check peer connections
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo 2>/dev/null | jq '{connections, connections_in, connections_out}'
```

### One-Liner Health Check

```bash
echo "=== Service ===" && systemctl is-active dogecoind && echo "=== Sync ===" && dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getblockchaininfo 2>/dev/null | jq -r '"Blocks: \(.blocks)/\(.headers) (\(.verificationprogress * 100 | floor)%)"' && echo "=== Peers ===" && dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo 2>/dev/null | jq -r '"Connections: \(.connections) (in: \(.connections_in), out: \(.connections_out))"'
```
...
**Check logs:**
```bash
sudo journalctl -u dogecoind -n 100
sudo tail -100 /var/log/dogecoin/debug.log
```
...
1. **Port already in use:**
   ```bash
   sudo ss -tlnp | grep -E "22556|22555"
   # Kill any rogue processes
   sudo pkill -9 dogecoind
   sudo systemctl start dogecoind
   ```

2. **Permission issues:**
   ```bash
   sudo chown -R dogecoin:dogecoin /var/lib/dogecoin
   sudo chown -R dogecoin:dogecoin /var/log/dogecoin
   sudo chown -R dogecoin:dogecoin /etc/dogecoin
   sudo chmod 700 /var/lib/dogecoin
   sudo chmod 700 /var/lib/dogecoin/wallets
   ```
...
4. **Corrupted database:**
   ...
   sudo mv /var/lib/dogecoin/chainstate /var/lib/dogecoin/chainstate.bak
   sudo systemctl start dogecoind
   
   # Or for a complete resync (slower but more thorough):
   # sudo rm -rf /var/lib/dogecoin/blocks /var/lib/dogecoin/chainstate
   ```

5. **Configuration error:**
   ```bash
   # Check config syntax
   sudo cat /etc/dogecoin/dogecoin.conf
   
   # Look for error messages about config
   sudo journalctl -u dogecoind | grep -i "error\|invalid\|failed"
   ```
...
4. **Autogen/configure failures:**
   ```bash
   # Clean and retry
   cd /usr/local/src/dogecoin
   make distclean 2>/dev/null
   ./autogen.sh
   ./configure --prefix=/opt/dogecoin \
...
1. **Spinning HDD (not SSD):**
   - Dogecoin requires high random I/O
...
2. **Low peer connections:**
   ```bash
   # Check peer count
   dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo | jq '.connections'
   
   # Should be 8+ for good sync speed
   # If < 8, check port 22556 forwarding
   ```

3. **Port 22556 not accessible:**
   ```bash
   # Check UFW
   sudo ufw status | grep 22556
   
   # Check if accepting incoming
   dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getnetworkinfo | jq '.connections_in'
...
**Check RPC binding:**
```bash
grep -E "rpcbind|rpcport|rpcuser" /etc/dogecoin/dogecoin.conf
# Mining pool mode: rpcbind=127.0.0.1 (localhost only)
```

**Check RPC is listening:**
```bash
sudo ss -tlnp | grep 22555
```

**Check firewall:**
```bash
sudo ufw status
# For remote RPC, port 22555 must be allowed
```

**Test RPC directly:**
```bash
# Use the correct credentials from config
RPC_USER=$(grep rpcuser /etc/dogecoin/dogecoin.conf | cut -d= -f2)
RPC_PASS=$(grep rpcpassword /etc/dogecoin/dogecoin.conf | cut -d= -f2)
...
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getblockchaininfo | jq '.blocks'
sleep 60
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getblockchaininfo | jq '.blocks'
```
...
```bash
sudo systemctl stop dogecoind
sudo rm /var/lib/dogecoin/peers.dat
sudo systemctl start dogecoind
```

**Add manual peers:**
```bash
# Add to /etc/dogecoin/dogecoin.conf
...
```bash
df -h /var/lib/dogecoin
du -sh /var/lib/dogecoin/*
```

**Clean up old logs:**
```bash
sudo truncate -s 0 /var/log/dogecoin/debug.log
```
...
Dogecoin can use 1-4GB RAM depending on configuration. Solutions:

1. **Reduce database cache:**
   ```bash
   # Edit /etc/dogecoin/dogecoin.conf
...
dogecoin-cli -conf=/etc/dogecoin/dogecoin.conf getblockchaininfo | jq '.initialblockdownload'
...
# Stop any other dogecoind or dogecoin-cli processes
sudo pkill dogecoind
sudo systemctl start dogecoind
```

**Lost wallet password:**
- The password is stored in `/etc/dogecoin/pool-wallet.conf`
...
# Stop service
sudo systemctl stop dogecoind
sudo systemctl disable dogecoind

# Remove config (not blockchain!)
sudo rm -rf /etc/dogecoin
sudo rm -f /etc/systemd/system/dogecoind.service
sudo rm -f /usr/local/bin/dogecoin*
sudo rm -rf /opt/dogecoin

# Remove user
sudo userdel dogecoin 2>/dev/null

# Reload systemd
sudo systemctl daemon-reload

# Re-run setup (will reuse existing blockchain in /var/lib/dogecoin)
sudo ./setup-dogecoin.sh
```
...
sudo ./uninstall-dogecoin.sh --force

# Or manual commands:
sudo systemctl stop dogecoind
sudo systemctl disable dogecoind
sudo rm -rf /opt/dogecoin /etc/dogecoin /var/log/dogecoin /var/lib/dogecoin
sudo rm -rf /usr/local/src/dogecoin /usr/local/BerkeleyDB.4.8
sudo rm -f /etc/systemd/system/dogecoind.service /usr/local/bin/dogecoin*
sudo userdel dogecoin 2>/dev/null
sudo systemctl daemon-reload
echo "Done - Dogecoin completely removed"
```
...
```bash
# Service logs
sudo journalctl -u dogecoind | grep -i error | tail -20

# Debug log
sudo grep -i "error\|warning\|fatal" /var/log/dogecoin/debug.log | tail -50
...
```ini
# /etc/dogecoin/dogecoin.conf optimizations
...
1. **Check logs carefully:**
   ```bash
   sudo journalctl -u dogecoind -n 200 | grep -i error
   ```

2. **Dogecoin community:**
   - [Reddit r/Dogecoin](https://www.reddit.com/r/dogecoin/)
   - [Dogecoin Discord](https://discord.gg/dogecoin)
   - [Dogecoin GitHub Issues](https://github.com/dogecoin/dogecoin/issues)

3. **Include in your question:**
   - Output of `sudo systemctl status dogecoind`
   - Last 50 lines of logs
   - Your config (remove any passwords!)
   - Debian version: `cat /etc/debian_version`
   - Dogecoin version: `dogecoind --version`
   - Disk space: `df -h /var/lib/dogecoin`
